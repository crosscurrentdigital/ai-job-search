...
- **Check the industry qualifier on experience requirements literally, not just the function.** A posting can require "4+ years in enablement/change management/L&D **within a technology or professional services organization**" - the functional match (training, enablement) is not enough if the industry context doesn't match. Vitamin World (Senior Training Manager, 10 years) is retail, not tech or professional services - it demonstrates the *function* strongly but does not satisfy an industry-scoped requirement like this one. Score industry-scoped requirements against the actual industry, and flag the mismatch explicitly rather than letting a strong functional match paper over it. (Candidate's own ventures - Crucible Lab / 102:18 INC - are tech/product-studio adjacent and partially close this gap, but it is a real partial gap, not a clean match.)
- **Revenue/growth-adjacent enablement roles (e.g. GitLab's "AI Transformation Owner, CRO") carry a specific soft risk: thin personal SaaS traction.** CrossCurrent Digital (~60 subscribers after 5 years) and WatchSafe.app (not yet gaining traction) are real, not fabricated, weak points. This does not disqualify enablement/builder-type roles (they ask whether the candidate can build tooling that helps *other people's* sales/marketing execution, not whether he personally scaled a product), but an interviewer may reasonably probe it, especially for CRO-org or growth-facing roles. Score it as a **discussion risk to prepare for**, not an automatic Experience Match deduction - the prepared answer (own the gap, tie it to BizPilotAI.app's origin story) neutralizes most of the risk if used.
